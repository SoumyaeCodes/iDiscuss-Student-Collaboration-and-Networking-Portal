...
<?php
$sql="SELECT COUNT(rating_action) as `likerate`,`comment`, `comment_sender_name` 
FROM tbl_comment, rating_info WHERE rating_action='dislike' AND tbl_comment.comment_id=rating_info.post_id";
                                    ...
                                    <table id="users" class="table table-bordered table-striped">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Replying To Question</th>
                                                <th>Dislikes</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <?php 
                                            $sql_comment_id= "SELECT `comment_id` FROM `tbl_comment`";
                                            ...
                                            while($row1=mysqli_fetch_assoc($result_comment_id))
                                            {
                                                ...
                                                while($row1=mysqli_fetch_assoc($sql_post_id))
                                                {
                                                    ...
                                                    while($row1=mysqli_fetch_assoc($sql_que_title))
                                                    {
                                                        ...
                                                        $sql_ans_like="SELECT COUNT(rating_action) as `dislikerate`, `comment`, `comment_sender_name` FROM `rating_info`,`tbl_comment` WHERE `comment_id`=$comment_id AND rating_action='dislike' AND rating_info.post_id=tbl_comment.comment_id";
